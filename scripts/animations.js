/* ============================================
    ANIMATIONS.JS - Gestión de animaciones visuales
    Contenido: Burbujas, estrellas, corazones y efectos especiales
   ============================================ */

/* === BURBUJAS FLOTANTES (MODO DÍA) === */
function createBubbles() {
	// Limpiar burbujas existentes para evitar duplicados
	document.querySelectorAll('.bubble').forEach(bubble => bubble.remove());

	// Burbujas grandes (4 fijas con posiciones predeterminadas)
	...
	// Burbujas pequeñas (10 aleatorias distribuidas por la pantalla)
	for (let i = 0; i < 10; i++) {
...
function createStars() {
	// Limpiar estrellas previas para evitar duplicados
	removeStars();
	
	// Crear o reutilizar contenedor de estrellas
	...
	// Calcular cantidad de estrellas basado en área de pantalla
	// Optimización: menos estrellas = mejor rendimiento en scroll
	const area = window.innerWidth * window.innerHeight;
	const base = Math.floor(area / 5500);
	const starCount = Math.max(60, Math.min(300, base)); // Limitar entre 60-300 estrellas
	
	// Usar fragment para optimizar DOM manipulation
	...
	for (let i = 0; i < starCount; i++) {
		...
		// Variables CSS para animación de parpadeo
...
function createHearts() {
	...
	for (let i = 0; i < 15; i++) {
		...
		// Auto-eliminar corazón después de la animación
		setTimeout(() => {
			heart.remove();
		}, 2500);
	}
...
function ensureHeartbeatIndicator() {
	// Crear indicador solo si no existe
...
function checkHeartbeat(message) {
	// Palabras clave que activan el efecto de latido
